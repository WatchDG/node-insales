...
export class InSales {
    ...
    async updateDeliveryVariant(id: DeliveryVariantId, update: DeliveryVariantUpdate) {
        try {
            const {data} = await this.instance.put(`/admin/delivery_variants/${id}.json`, update);
            return ResultOk(data);
        } catch (error) {
            return ResultFail(error);
        }
